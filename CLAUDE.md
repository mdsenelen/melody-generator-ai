...
- Transcription is always chunked (the `TRANSCRIBE_CHUNKED` flag and the old single-pass truncating path were removed in plan step 4): the whole upload is transcribed in overlapping 30s windows, so peak memory is length-independent (~400MB, measured — `docs/PLAN-full-transcription-split.md`). `/api/transcribe`'s result is **MIDI + `note_events` only** — mood/key/tempo/chords/histogram moved to `POST /api/analyze`, which slices `note_events` to a user-chosen clip window (no librosa, no Basic Pitch). `MAX_ANALYSIS_DURATION_SEC` (60s) now only bounds the single-pass decode still used for browser webm recordings and the generation path.
...
- **Free tier (512MB) is a permanent, accepted constraint — a deliberate cost decision, do not propose paying to lift it.** The consequence, enforced in code: **only one heavy audio task runs at a time.** `HEAVY_WORK_LOCK` serializes the transcribe worker against every generation route; a generation request that can't get the lock returns **429** immediately (not a block, not an OOM); the worker waits a bounded `WORKER_HEAVY_WORK_WAIT_SEC` then re-queues. Generation also 503s fast (before importing torch) when model weights aren't present. The joint checkpoint (~1.4 MB) is not in git — the deployed backend fetches it from `MODEL_WEIGHTS_URL` on the first generation request and caches it on the ephemeral disk (`_ensure_joint_weights`, stdlib download, no torch). With that env var set, generation *is* active in prod — torch gets imported, so the memory headroom the rest of this section guards is thinner during a generation; `HEAVY_WORK_LOCK` still serializes it against transcription. See the skill's "Cost constraints" section — those limits are the *reason the architecture looks the way it does*, and they're live, not historical.
- **Known limitation — the "poisoned floor" (measured, not fixable in code).** The first generation imports `torch` (~150–170 MB RSS) and it stays resident for the container's life. `gc` + `malloc_trim` recover **nothing** (prod: RSS held at ~455 MB for 5+ min after a generation; local probe: 196 MB → 196 MB). Purging `torch` from `sys.modules` also recovers nothing **and segfaults on the next import** — torch's global state (dispatcher, thread pools) can't be re-initialised. So after any generation, a transcription on that same warm container has ~57 MB of headroom instead of ~200 and **may OOM**. It's not data loss: the stale-job reclaim + `max_attempts=3` re-run it on a fresh (floor-reset) container, so the user sees one slow retry. The real fix (deferred until generation traffic is real) is to run the torch work in a short-lived subprocess that exits and hands the memory back. Don't re-attempt in-process torch unloading — it's been measured.
