...
- No formal SLA. The Render **Standard** backend does not idle-suspend, so routine cold starts are gone; a restart (deploy, or the rare crash) is still possible and must degrade gracefully — a visible loading state and the frontend's retry-once on job creation, not a hung request or a raw 502. Vercel's hobby frontend is effectively always warm.
...
The backend web service is on Render's **Standard plan** ($25/month — 2GB RAM, 1 CPU, no idle-suspend) as of **2026-09-07**. The frontend stays on Vercel's hobby tier ($0). The job stack (Postgres/Redis/object storage) stays entirely on free tiers.

> **The free-tier era (through 2026-09-06) — kept because it explains the current architecture.** Everything below the line describes the 512MB free web-service tier the project ran on before the Standard move. Those limits are no longer live, but three of them left permanent marks: (1) transcription is chunked (`TRANSCRIBE_CHUNKED=true`) so peak memory is length-independent — see "Chunked transcription" in `docs/PROGRESS.md` and `docs/PLAN-full-transcription-split.md`; (2) `torch` and the CVAE/IDDM model classes are lazy-imported so the transcription path never pays for them; (3) `HEAVY_WORK_LOCK` serializes the transcribe worker against the generation routes, and `_release_memory_to_os()` (`malloc_trim`) runs after each. All three are still correct on Standard, just with comfortable margin now rather than as survival measures. Measured on Standard 2026-09-06: a chunked 5-minute transcription peaks at ~400-430MB, flat across chunks, well under the 2GB ceiling.

---

- Render free web services spin down after 15 minutes idle and have **ephemeral disk** — anything written to local disk (SQLite job DB, local job storage, uploaded/generated audio) is lost on restart or redeploy. This is the concrete reason the dev-only fallback stack cannot be what real users hit. (Standard does not idle-suspend; the ephemeral-disk point still holds and is still the reason for the external job stack.)
- The job stack (Postgres, Redis, object storage) is live as of 2026-08-24 and entirely on free tiers: **Neon** (external managed Postgres, not Render's own Postgres offering), Render's free **Key Value** tier for Redis, and Backblaze **B2**'s free tier for object storage — see "Production Readiness" below. `RUN_WORKER_IN_PROCESS` stays `true` (the worker runs in-process on the web service); splitting a dedicated Background Worker out is possible but not currently done — the 2GB instance has the headroom.
- Basic Pitch transcription runs at roughly real-time speed on the CPU here — a burst of concurrent real users will queue up visibly (the worker processes one job at a time, and `HEAVY_WORK_LOCK` serializes it against generation). Rate limiting (live, above) and that serialization are both cost- and latency-relevant, not just abuse-prevention.
- **Historical (free tier, 2026-08-24 measurement): the 512MB memory ceiling was a sharper constraint than CPU speed once the worker ran in-process.** A cold instance's memory climbed from near-zero to ~530MB (of a 536MB limit) over the ~4 minutes to process a *single* transcription (torch/librosa/basic-pitch/music21 all importing for the first time + the analysis work), finishing just under the ceiling; a *second* concurrent request reliably got OOM-killed mid-run. This is what drove the lazy-torch split, chunked transcription, and `HEAVY_WORK_LOCK` — and then the Standard move once even the single-request path sat too close to the line. On Standard the same workload has ~1.6GB of headroom.
...
**Two paths now, gated on `TRANSCRIBE_CHUNKED` (prod: `true`).**

- **Chunked (`_transcribe_and_mood_chunked`):** transcribes the *whole* upload in overlapping 30s windows (`_merge_chunk_notes` welds notes across boundaries — see `docs/PLAN-full-transcription-split.md`), so the output MIDI is full-length and peak memory is length-independent. The mood/key/BPM/chords analysis runs on the first `MAX_ANALYSIS_DURATION_SEC` (default **120s**, env-overridable) only. A `MAX_UPLOAD_DURATION_SEC` (default 600s / 10 min) cap rejects longer uploads. `docs/PLAN-full-transcription-split.md` steps 2–3 split this into `/api/transcribe` (full MIDI) + a separate synchronous `/api/analyze` (librosa-only) with user-chosen clip bounds, and rename `MAX_ANALYSIS_DURATION_SEC` to a "default clip" constant.
- **Single-pass (`_transcribe_and_mood`, legacy — removed in plan step 4):** `_read_audio_bytes` truncates decoded audio to `MAX_ANALYSIS_DURATION_SEC` before any analysis, probing the header via `soundfile` (no decode) to bound `librosa.load` itself. The chunked path falls back to this only when duration can't be probed (webm mic recordings).

Both include `source_duration_sec` (full upload duration) and `truncated: bool` alongside `duration_sec` in the result; chunked always reports `truncated: false`.
...
**`RUN_WORKER_IN_PROCESS`** (default `true`) controls whether `main.py`'s lifespan starts the worker loop on a background thread inside the web process **and** whether it eagerly warms up (loads) Basic Pitch at startup -- both are gated on this flag, so `RUN_WORKER_IN_PROCESS=false` means the web process does neither. Locally, `uvicorn app.main:app --reload` alone is enough. **Production also runs it `true`** — a single Standard (2GB) web service running both the API and the worker thread. Splitting a dedicated Background Worker out (`RUN_WORKER_IN_PROCESS=false` on the web service + `python -m app.worker_main` as its own service) is the cleaner shape and stays a real option, but it's not currently done: `HEAVY_WORK_LOCK` already serializes inference against the API's generation routes, and the 2GB instance has the headroom. (Nothing outside the worker calls `run_basic_pitch` — the legacy synchronous transcribe route is gone — so `RUN_WORKER_IN_PROCESS=false` genuinely means the web process never runs the pipeline, whenever the split is worth doing.)
...
- **Analysis clip length:** `MAX_ANALYSIS_DURATION_SEC` env var, default **120s**. With chunked transcription (`TRANSCRIBE_CHUNKED=true`) this no longer truncates the *transcription* — the whole upload is transcribed — it's the window the mood/key/BPM/chords analysis runs on. Plan steps 2–3 rename it and make the clip bounds a user choice in the UI. `MAX_UPLOAD_DURATION_SEC` (default 600s) is the hard cap on upload length. `jobs/worker.py` renews the job lease per chunk (`JobStore.heartbeat`, `HEARTBEAT_LEASE_SECONDS` = 180s) so a long chunked job doesn't outlive `DEFAULT_LEASE_SECONDS` (900s) and get re-claimed mid-run.
- **Generation timeout:** `GENERATION_TIMEOUT_SECONDS` env var, defaults to 60s. Render's platform-level request timeout sits close to 100s, so this needs real margin below it (not just to be lower) or Render's raw CORS-header-less 502 wins the race instead of our clean JSON error. Less acute on Standard than on the CPU-starved free tier, but keep the margin. This now applies only to the *other* synchronous routes (`/generate-variants`, etc) — transcription no longer runs inside a request this bounds, since its old synchronous route was retired in favor of the async job workflow. `pollTranscribeJob`'s ~15 minute client-side poll budget is a separate, unrelated number: how long the *browser* waits across possibly-many polls, sized with the same margin above `MAX_ANALYSIS_DURATION_SEC` as the worker's lease. The frontend retries a failed `/api/upload` or `/api/transcribe` (job creation) call once, since a cold Render instance can fail the very first request outright while it's still waking up, and a retry right after is effectively always fast. Job creation is safe to retry unconditionally (no 504 carve-out needed, unlike the old synchronous route): it's just a fast enqueue, and the backend's idempotency key (see "Async Transcription Job Workflow" above) dedupes a retried creation instead of spinning up a redundant job.
...
- **Async transcription job workflow** (see "Async Transcription Job Workflow" above) — all optional in dev; omitting all of them gives the **dev-only** fallback setup (SQLite + in-process queue + local disk + in-process worker thread) — not fit for real-user production traffic (see "Non-Functional Requirements" and "Production Readiness"):
  - `RUN_WORKER_IN_PROCESS` — `true` (default, and what production runs) starts the worker loop + Basic Pitch warm-up inside the web process. `false` + `python -m app.worker_main` as a separate service is the split-out option, not currently deployed.
...
- **Backend web service:** Docker (`backend/Dockerfile`) — Python 3.10-slim, exposes port 8000. Render **Standard** plan, Frankfurt region. Runs `RUN_WORKER_IN_PROCESS=true` (API + worker thread in one service) — see "Async Transcription Job Workflow" above for the split-out option if a dedicated worker becomes worth it.
...
- **Worker service — not provisioned, deliberately.** Only one Render service exists (`melody-generator-ai`, the web service, Standard plan); there is no second Background Worker service, and `RUN_WORKER_IN_PROCESS` stays at its default `true`. On the old free tier this was a survival tradeoff with a real cost (see "Cost constraints" — the in-process worker couldn't serve two transcriptions back-to-back on 512MB). On Standard (2GB) the in-process arrangement has headroom and `HEAVY_WORK_LOCK` serializes inference against the API's generation routes. Splitting the worker out stays a clean option for later, not a fix that's overdue.
