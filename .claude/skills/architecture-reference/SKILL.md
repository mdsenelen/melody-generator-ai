...
- **Transcription memory is length-independent.** The chunked path (`_transcribe_full_chunked` + `_merge_chunk_notes`) transcribes the whole upload in overlapping 30s windows, so the posteriorgram never exceeds one chunk. Measured: a 5-minute clip peaks at ~400–430MB, flat across chunks. See `docs/PLAN-full-transcription-split.md`.
- **`torch` is lazy-imported.** `inference.py` and `colab_parity`'s model classes don't load torch at module import — only the generation paths do, and only after `_ensure_joint_weights()` + the weights-exist check. `_ensure_joint_weights()` (stdlib `urllib`, no torch) fetches `joint_e2e_weights.pth` (~1.4 MB, not in git) from `MODEL_WEIGHTS_URL` on the first generation request and caches it on the ephemeral disk (re-fetched after a redeploy). With `MODEL_WEIGHTS_URL` unset, or a failed download, generation is a ~1ms 503 that never touches torch. With it set, generation *is* live in prod and torch becomes resident during a generation — `HEAVY_WORK_LOCK` serializes it against transcription, but headroom on 512 MB is thin; treat generation as best-effort there. (Lazy-torch verified by measurement in the Phase B audit.)
...
| `model/weights/` | `.pth` checkpoint files — not committed (gitignored). Prod fetches `joint_e2e_weights.pth` from `MODEL_WEIGHTS_URL` on first generation use (`_ensure_joint_weights`); loaded at runtime with graceful 503 fallback |
...
**Transcription is always chunked** (`_transcribe_and_mood_chunked` → `_transcribe_full_chunked`; the `TRANSCRIBE_CHUNKED` flag and the single-pass truncating path were removed in plan step 4). The whole upload is transcribed in overlapping 30s / 4s-overlap windows (`TRANSCRIBE_CHUNK_SEC` / `TRANSCRIBE_OVERLAP_SEC`), decoded one window at a time via `librosa.load(offset=, duration=)`, so peak memory doesn't grow with clip length. `_merge_chunk_notes` welds notes across boundaries — see `docs/PLAN-full-transcription-split.md` for the algorithm (the risky part). Output MIDI is full-length, `truncated` always `false`. `MAX_UPLOAD_DURATION_SEC` (600s / 10 min) is the hard cap. The worker renews the job lease per chunk (`JobStore.heartbeat`) so a long job isn't reclaimed mid-run.

`_transcribe_and_mood` (the old single-pass fn) is **not deleted** — it's still the fallback for un-probeable input (browser webm/opus recordings soundfile can't read the header of) and the transcription step inside `generate_iddm_variants`. It caps decode to `MAX_ANALYSIS_DURATION_SEC` (60s).

**`/api/transcribe`'s result is MIDI + `note_events` only** (plan step 5, contract step 3): `n_notes`, `duration_sec`, `source_duration_sec`, `truncated`, `midi_b64`, `midi_filename`, `wav_b64: null`, `wav_filename: ""`, `n_chunks`, `note_events` (trimmed `{start,end,pitch,velocity}`). Mood / key / tempo / chords / pitch histogram / average pitch **moved to `POST /api/analyze`** — `{job_id, clip_start_sec?, clip_end_sec?}` → `analyze_clip()` slices the stored `note_events` to the window and computes them with pure arithmetic (`_estimate_tempo` / `_key_from_histogram` / `heuristic_mood_from_metrics` / `_pitch_histogram` / `_chords_from_note_events`). No librosa, no Basic Pitch, no `HEAVY_WORK_LOCK`; re-runnable against any window.
...
- **`MAX_ANALYSIS_DURATION_SEC`** env var, default 60s. It no longer bounds anything on `/api/transcribe` (the whole upload is transcribed; analysis moved to `/api/analyze`, whose window is a frontend choice). It now only caps the single-pass `_transcribe_and_mood` decode — the webm-recording fallback and the generation path. `MAX_UPLOAD_DURATION_SEC` (600s) is the hard upload cap; `TRANSCRIBE_CHUNK_SEC` (30) / `TRANSCRIBE_OVERLAP_SEC` (4) are the chunk geometry. `jobs/worker.py` renews the lease per chunk (`heartbeat`, `HEARTBEAT_LEASE_SECONDS` 180s) so a long chunked job doesn't outlive `DEFAULT_LEASE_SECONDS` (900s).
...
- **Backend web service:** Docker (`backend/Dockerfile`) — Python 3.10-slim, port 8000, `apt` deps `fluidsynth` + `ffmpeg`. Render **free** tier, Frankfurt. Runs `RUN_WORKER_IN_PROCESS=true` (API + worker in one service); `HEAVY_WORK_LOCK` keeps that safe on 512MB (one heavy task at a time). Split the worker out later if wanted — see "Async Transcription Job Workflow" above.
